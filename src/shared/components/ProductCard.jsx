...
const ProductCard = React.memo(function ProductCard({ id, image, hoverImage, title, price, delivery, rating, rounded = 'lg', roundedTop, roundedBottom, outOfStock = false }) {
  ...
  const roundedClass = (rounded === 'none' || rounded === 'square')
    ? ''
    : (typeof rounded === 'string' && ROUNDED_CLASSES[rounded] != null)
      ? ROUNDED_CLASSES[rounded]
      : ROUNDED_CLASSES.lg

  const roundedTopClass = (imageRounded === 'none' || imageRounded === 'square')
    ? ''
    : (typeof imageRounded === 'string' && ROUNDED_TOP_CLASSES[imageRounded] != null)
      ? ROUNDED_TOP_CLASSES[imageRounded]
      : ROUNDED_TOP_CLASSES.lg

  const roundedBottomClass = (imageRounded === 'none' || imageRounded === 'square')
    ? ''
    : (typeof imageRounded === 'string' && ROUNDED_BOTTOM_CLASSES[imageRounded] != null)
      ? ROUNDED_BOTTOM_CLASSES[imageRounded]
      : ROUNDED_BOTTOM_CLASSES.lg

  const infoRoundedBottomClass = (infoRounded === 'none' || infoRounded === 'square')
    ? ''
    : (typeof infoRounded === 'string' && ROUNDED_BOTTOM_CLASSES[infoRounded] != null)
      ? ROUNDED_BOTTOM_CLASSES[infoRounded]
      : ROUNDED_BOTTOM_CLASSES.lg

  const cardStyle = isNumeric ? { borderRadius: `${rounded}px` } : {}
  const imageTopStyle = imageIsNumeric && imageRounded > 0
    ? { borderTopLeftRadius: imageRounded, borderTopRightRadius: imageRounded }
    : {}

  const imageBottomStyle = imageIsNumeric && imageRounded > 0
    ? { borderBottomLeftRadius: imageRounded, borderBottomRightRadius: imageRounded }
    : {}

  const infoBottomStyle = infoIsNumeric && infoRounded > 0
    ? { borderBottomLeftRadius: infoRounded, borderBottomRightRadius: infoRounded }
    : {}
