...
function OrdersPage() {
  ...
  const getStatusDisplay = (oi) => {
    ...
    const deliveredAt =
      oi.item?.deliveredAt ??
      oi.latestStatusHistory?.createdAt ??
      oi.orderCreatedAt;
    ...
    if (
      ...
      return {
        type: "exchange_process",
        label: "EXCHANGE IN PROCESS",
        statusLabel,
        dateStr,
        name,
        fullAddress,
  ...
  return (
    <div className="min-h-screen bg-white text-black pt-24 pb-12 font-sans">
      <div className=" px-4 sm:px-6 md:px-8 ">
        <h1 className="text-xl sm:text-2xl font-bold uppercase tracking-wider text-gray-800 mb-6 sm:mb-8">
          My orders
        </h1>
        ...
        {orderSuccessFromState && orderIdFromState && (
          <div className="mb-6 sm:mb-8 p-4 sm:p-6 border border-green-200 bg-green-50 rounded-lg">
            <p className="font-semibold text-green-800 uppercase text-sm sm:text-base">
              Order placed successfully
            </p>
            <p className="mt-1 text-xs sm:text-sm text-gray-700">
              Order ID:{" "}
              <span className="font-mono font-medium">{orderIdFromState}</span>
            </p>
            <div className="mt-4 flex flex-wrap gap-2 sm:gap-3">
              <Link
                to={ROUTES.SEARCH}
                className="inline-block px-4 py-2 bg-black text-white text-xs sm:text-sm font-medium uppercase hover:bg-gray-800"
              >
                Continue shopping
              </Link>
              <Link
                to={ROUTES.HOME}
                className="inline-block px-4 py-2 border border-black text-black text-xs sm:text-sm font-medium uppercase hover:bg-gray-50"
              >
                Back to home
              </Link>
            </div>
        ...
        {loading ? (
          ...
          <div className="border border-gray-300 rounded-lg overflow-hidden">
            ...
            {sortedOrderItems.map((oi, idx) => {
              ...
              const itemId =
                oi.itemId?.toString?.() ?? oi.productItemId?.toString?.() ?? "";
              const productId = oi.itemId || item._id;
              const rowKey =
                orderId && itemId
                  ? `${orderId}-${itemId}-${idx}`
                  : `row-${idx}`;
              ...
              return (
                ...
                >
                  ...
                  <Link
                    ...
                    className="flex gap-3 sm:gap-4 min-w-0 md:col-span-6 hover:bg-gray-50 p-1 rounded transition"
                  >
                    {" "}
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="font-bold text-gray-900 uppercase text-xs sm:text-sm truncate">
                        {brand}
                      </p>
                      <p className="text-gray-700 text-xs sm:text-sm mt-0.5 normal-case line-clamp-2">
                        {name}
                        {color ? ` ${color}` : ""}
                      </p>
                      {trackingId && (
                  ...
                  <div className="flex md:contents flex-wrap gap-x-4 gap-y-1">
                    <div className="md:col-span-2 flex items-center md:justify-center text-gray-800 text-sm">
                      <span className="md:hidden text-gray-500 font-medium mr-1">
                        Qty:
                      </span>
                      {quantity}
                    </div>
                  ...
                  <div className="md:col-span-2 mt-1 md:mt-0">
                    {statusDisplay.type === "track" ? (
                      <div className="space-y-2">
                        <p className="font-bold text-gray-900 uppercase text-[11px] sm:text-xs">
                          ...
                          className={`font-bold uppercase text-[11px] sm:text-xs px-2 py-1 rounded inline-block border ${
                            statusDisplay.type === "cancelled"
                              ...
                              : statusDisplay.type === "delivered"
                                ...
                                : statusDisplay.type === "exchanged" ||
                                    statusDisplay.type === "exchange_process"
                          ...
                          statusDisplay.type !== "cancelled" && (
                            <>
                              ...
                              {statusDisplay.fullAddress &&
                                statusDisplay.fullAddress !== "—" && (
                                  <p className="text-gray-500 text-[10px] sm:text-xs mt-0.5 break-words">
                                    {statusDisplay.fullAddress}
                                  </p>
                                )}
                            </>
        ...
        <div className="mt-6 sm:mt-8">
          <Link
            to={ROUTES.HOME}
            className="text-xs sm:text-sm font-medium uppercase text-gray-700 hover:text-black hover:underline"
          >
            ← Back to home
          </Link>
        </div>
